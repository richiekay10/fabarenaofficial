...
const navItems: NavItem[] = [
  ...
  { label: 'My Customers', icon: Users, route: { name: 'agent-customers' }, path: '/agent-customers', match: ['agent-customers'] },
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 bg-slate-900 flex flex-col transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-800">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Field Agent Portal</p>
          </div>
        </div>
        ...
        <div className="border-t border-slate-800 p-3">
          <div className="flex items-center gap-3 px-2 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400">Field Agent{profile?.zone ? ` · ${profile.zone}` : ''}</p>
            </div>
          </div>
          <button
            onClick={signOut}
